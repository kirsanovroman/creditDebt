...
class InviteService:
    ...
    async def accept_invite(
        ...
        Raises:
            ValueError: Если токен невалиден, использован, или долг не найден, или пользователь уже является кредитором
        
        Note:
            Срок действия приглашения не проверяется - приглашения действуют бессрочно.
            Для тестирования разрешено принимать приглашение на свой же долг (должник может быть кредитором).
        """
        ...
        # Проверяем, что пользователь не является уже кредитором
        # Убрали проверку на должника для возможности тестирования
